...
- 把多个 GitHub 仓库组成一个档案库，并按设备查看全部档案
- 多台设备可以共用同一个仓库池
- 支持整包恢复
- 支持按文件夹或文件恢复
- 支持 `合并恢复` 和 `严格覆盖`
...
可以在 `仓库设置` 里切换到 `data` 下的其它用户目录。创建备份、整包恢复、按路径恢复、自动备份和保留策略都会使用当前选择的用户目录。
...
2. 在 `仓库设置` 里点击 `添加仓库`。
3. 粘贴完整 GitHub 仓库链接或填写 `owner/repo`，再填写可访问该仓库的 Token。
4. 设置当前设备名和备份用户目录并保存。
5. 切到 `创建备份`，确认首次写入仓库后点击 `创建整包备份`。

仓库第一次承载备份时需要建立自己的隐藏分块，因此上传较多。
切回使用过的仓库会继续复用其中已有分块。

### 添加和切换仓库

- 在 `仓库设置` 中点击 `添加仓库`，即可扩充仓库池。
- 新仓库可以沿用第一个仓库的 Token；只有权限不同时才需要填写单独 Token。
- 已有仓库池中的成员也必须逐个手动添加；输入一个旧仓库不会自动导入远端其它仓库。
- 多仓库不是自动均衡。一份备份始终完整存放在一个仓库中。
- 仓库行右侧的删除按钮只删除本机配置，不删除 GitHub 仓库或远端备份。
- `当前写入仓库` 决定这台设备接下来创建备份的位置；切换不会移动历史档案。
- 档案库、恢复、下载、检查和维护统计会读取仓库池中的全部可用仓库。
...
1. 设备 A 上传备份
2. 设备 B 手动添加一个已有仓库，并提供可访问该仓库的 Token
3. 插件识别该仓库所属的仓库池后，设备 B 打开 `档案库`
4. 如需访问其它成员仓库，再逐个手动添加；插件不会自动导入

同一个仓库可以同时作为多台设备的 Archive Reserve 仓库，不会因为设备不同而被拒绝。
...
- `刷新空间`：查看整个仓库池占用，并按仓库显示实际占用、按设备显示档案数据量
- `立即回收`：清理不再被任何档案引用的旧分块
- `检查`：检查某个档案是否完整可恢复

设备数据量按各份档案记录的原始数据累加；由于不同档案可以复用分块，它不等于新增的 GitHub 实际占用。实际占用以按仓库统计为准。
